  ...
  void scrollTextTask(void *pvParameters)
  {
    while (1)
    {
      if (displayActive)
      {
        ...
        if (titleWidth > SCREEN_WIDTH)
        {
          int titleStartX = SCREEN_WIDTH;
          int titleEndX = -titleWidth;

          while (titleStartX > titleEndX)
          {
            display.clearDisplay();
            EURK_setxy(titleStartX, 0);                           // 제목 출력 좌표 설정
            EURK_puts(const_cast<char *>(currentTitle.c_str()));  // 제목 출력
            EURK_setxy(0, 16);                                    // 가수 고정
            EURK_puts(const_cast<char *>(currentArtist.c_str())); // 가수 출력
            display.display();
            titleStartX -= 2; // 이동 속도 조절
            delay(SCROLL_DELAY);
          }
        }
        else
        {
          // 제목이 화면 너비를 초과하지 않는 경우 고정
          display.clearDisplay();
          EURK_setxy(0, 0);
          EURK_puts(const_cast<char *>(currentTitle.c_str()));
          EURK_setxy(0, 16);
